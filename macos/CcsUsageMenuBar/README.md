...
The app does not decide what a row says. `packages/usage-view` (the same module `ccs usage` and the hub read) turns the snapshot into sections and rows, and the app runs it through JavaScriptCore and only draws the result. `make-app.sh` regenerates the bundle before building:

```sh
bun build packages/usage-view/jsc-entry.ts --target browser --format iife \
  --outfile macos/CcsUsageMenuBar/Sources/CcsUsageMenuBar/Resources/usage-view.js
```

The generated `usage-view.js` is checked in so `swift test` runs without bun. After changing the module, rerun `make-app.sh` or the command above and commit the new bundle.

### Order and freshness

Drag a section header to reorder sections, or drag a row to reorder it within its section. Both orders persist in the app's defaults (`sectionOrder`, `rowOrder`) and go to the module as its `order` argument; anything new appends in its natural position.
...
Each Claude account shows its raw weekly Fable cap beside the all-models weekly pool. The Fable cap nests inside that pool, so it never drives the menu bar reading. That rule lives in `packages/usage-view/index.ts`, not in Swift.
