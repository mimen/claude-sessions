...
Malformed TOML, unknown model values, aliases, and `[1m]` input fail new-session preflight. A model-policy role also rejects `--via`; policy-less/manual roles keep existing `--via` behavior.
