...
import { requestUnixHttp, UnixHttpTimeoutError } from "./transport.ts";

test("Unix transport reports a distinct timeout error", async () => {
  const root = mkdtempSync(join(tmpdir(), "ccs-transport-timeout-"));
  ...
  try {
    await expect(requestUnixHttp(socketPath, "/", { timeoutMs: 10 })).rejects.toBeInstanceOf(
