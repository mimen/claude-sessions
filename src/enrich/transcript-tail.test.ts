...
describe("the arc", () => {
  ...
  test("drops user turns no human typed", async () => {
    // Measured on a real 558-message session: five of twelve arc entries were a screenshot temp
    // path, an interruption marker, and an injected skill definition.
    const lines = [
      message("user", "redesign the sidebar rows"),
      message("user", "[Request interrupted by user]"),
      message("user", "[Image: source: /var/folders/0r/T/Screenshot 2026-08-10.png]"),
      message("user", "Base directory for this skill: /Users/mimen/.claude/skills/chrome-agent"),
      message("user", "<system-reminder>background context</system-reminder>"),
      message("user", "Caveat: The messages below were generated by the user while running local commands"),
      ...Array.from({ length: 30 }, (_, i) => message("assistant", `turn ${i}`)),
    ];
    await withTranscript(lines, async (path) => {
      const tail = await readTranscriptTail(path, 10);
      expect(tail.arc).toContain("redesign the sidebar rows");
      expect(tail.arc.split("\n")).toHaveLength(1);
    });
  });

  test("keeps a request that merely opens with a marker", async () => {
    // The narrow-pattern rule: a marker with a request after it is a request.
    const lines = [
      message("user", "[Image #3] this shadow should cover the full row"),
      ...Array.from({ length: 30 }, (_, i) => message("assistant", `turn ${i}`)),
    ];
    await withTranscript(lines, async (path) => {
      const tail = await readTranscriptTail(path, 10);
      expect(tail.arc).toContain("this shadow should cover the full row");
    });
  });

  test("positions stay honest when noise is dropped", async () => {
    // Noise still counts toward the session's length, or a filtered prompt would appear to sit
    // earlier in the session than it does.
    const lines = [
      ...Array.from({ length: 50 }, () => message("user", "[Request interrupted by user]")),
      message("user", "the real question"),
      ...Array.from({ length: 30 }, (_, i) => message("assistant", `turn ${i}`)),
    ];
    await withTranscript(lines, async (path) => {
      const tail = await readTranscriptTail(path, 10);
      expect(tail.arc).toMatch(/^\[6[0-9]%\] the real question$/);
    });
  });
