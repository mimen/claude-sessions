...
/**
 * Text that arrives in the user role without a human having typed it.
 *
 * The arc claims to be the spine of what a person asked for, so this matters more here than
 * anywhere else in the file: on the first real session it was measured against, five of twelve
 * entries were a screenshot's temp path, an interruption marker, and an injected skill definition.
 * That is not a cosmetic problem — the budget spent on them is budget not spent on a prompt that
 * says what the session is about.
 *
 * Anchored at the start and deliberately narrow. A wrongly dropped entry costs one of 24 slots; a
 * pattern loose enough to swallow real prose would quietly delete the signal this block exists to
 * carry. `[Image #2] the shadow should cover the row` stays — a marker with a request after it is
 * a request.
 */
const HARNESS_NOISE: readonly RegExp[] = [
  ...
  /^\[Image:\s*source:/i,
  /^\[Pasted text/i,
  /^<(system-reminder|command-name|command-message|command-args|local-command-stdout)\b/i,
...
function isHarnessNoise(text: string): boolean {
  return HARNESS_NOISE.some((pattern) => pattern.test(text));
}
...
export async function readTranscriptTail(
  ...
  try {
    for await (const line of rl) {
      ...
      if (obj.type === "user" && !isHarnessNoise(rendered)) {
      ...
      // `total` counts every rendered message, noise included, so the percentages stay honest
      // about where in the session a prompt actually sits.
      total++;
