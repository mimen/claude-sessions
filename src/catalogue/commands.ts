...
/** Persist a legacy event/skill label in generic metadata, not a catalogue column. */
function setLegacyCategory(
  key: "event" | "skill",
  sessionArg: string | undefined,
  ...
  const off = flags.includes("--off");
  if (!off && (!value || !value.trim())) {
    console.error(`usage: ccs ${key} [<session-id>|.] <name> [--off]`);
    return 1;
  }
  ensureDataDir();
  const db = openCatalogue(CATALOGUE_PATH());
  try {
    setMeta(db, id, key, off ? null : value!.trim(), now());
    console.log(off ? `cleared ${key} on ${id.slice(0, 8)}…` : `${key} ${value!.trim()} → ${id.slice(0, 8)}…`);
  } finally {
    db.close();
  }
  return 0;
...
/** ccs event [<id>|.] <name> [--off] — set the event category. */
export function event(sessionArg: string | undefined, value: string | undefined, flags: string[]): number {
  return setLegacyCategory("event", sessionArg, value, flags);
}

/** ccs skill [<id>|.] <name> [--off] — set the backing skill category. */
export function skill(sessionArg: string | undefined, value: string | undefined, flags: string[]): number {
  return setLegacyCategory("skill", sessionArg, value, flags);
}
...
/** Set (or clear) the agent runtime. Unset resolves to claude-code. */
export function substrate(sessionArg: string | undefined, value: string | undefined, flags: string[]): number {
  const id = resolveSessionId(sessionArg);
  if (!id) return notInSession();
  const off = flags.includes("--off");
  if (!off && (!value || !value.trim())) {
    console.error("usage: ccs substrate [<session-id>|.] <value> | --off   (unset = claude-code)");
    return 1;
  }
  ensureDataDir();
  const db = openCatalogue(CATALOGUE_PATH());
  try {
    setSubstrate(db, id, off ? null : value!.trim(), now());
    console.log(off ? `cleared substrate on ${id.slice(0, 8)}… (back to ${DEFAULT_SUBSTRATE})` : `substrate ${value!.trim()} → ${id.slice(0, 8)}…`);
  } finally { db.close(); }
  return 0;
}

/** Stamp the launching identity; omitted value defaults to CLAUDE_IDENTITY. */
export function identity(sessionArg: string | undefined, value: string | undefined, flags: string[]): number {
  const id = resolveSessionId(sessionArg);
  if (!id) return notInSession();
  const off = flags.includes("--off");
  const identityValue = value?.trim() || process.env.CLAUDE_IDENTITY;
  if (!off && !identityValue) {
    console.error("usage: ccs identity [<session-id>|.] [<name>] [--off]   (name defaults to $CLAUDE_IDENTITY)");
    return 1;
  }
  ensureDataDir();
  const db = openCatalogue(CATALOGUE_PATH());
  try {
    setIdentity(db, id, off ? null : identityValue!, now());
    console.log(off ? `cleared identity on ${id.slice(0, 8)}…` : `identity ${identityValue} → ${id.slice(0, 8)}…`);
  } finally { db.close(); }
  return 0;
}
