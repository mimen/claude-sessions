...
export interface CatalogueRow {
  ...
  /** The launching identity (`CLAUDE_IDENTITY` exported by the launcher) — which
   * `claude-<name>` alias started this session. */
  identity?: string | null;
  /** Legacy categories retained in generic metadata for compatibility. */
  event?: string | null;
  skill?: string | null;
...
function migrate(db: Database): void {
  ...
  if (v < 32) {
    // The session ontology: runtime substrate and launching identity. Role already exists as
    // the catalogue's primary responsibility axis; these fields distinguish its embodiment.
...
export function rowFrom(r: Record<string, unknown> | null): CatalogueRow | null {
  ...
  const skill = typeof meta.skill === "string" ? meta.skill : null;
  return {
...
export function setSubstrate(db: Database, sessionId: string, substrate: string | null, now: string): void {
  // Store the default as unset so NULL and `claude-code` do not represent the same state.
