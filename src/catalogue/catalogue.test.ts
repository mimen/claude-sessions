...
test("concurrent opens wait for an active writer before confirming WAL", async () => {
  ...
  try {
    ...
    expect(concurrent.query("PRAGMA user_version").get()).toEqual({ user_version: 39 });
    concurrent.close();
...
test("repairs a current catalogue down-stamped to every historical version", () => {
  ...
  try {
    for (let version = 0; version < 39; version += 1) {
      ...
      expect(repaired.query("PRAGMA user_version").get()).toEqual({ user_version: 39 });
...
test("repairs partially migrated catalogues missing cluster and skill", () => {
  ...
  try {
    for (const version of [5, 30]) {
      ...
      expect(migrated.query("PRAGMA user_version").get()).toEqual({ user_version: 39 });
