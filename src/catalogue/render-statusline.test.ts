...
function row(over: Partial<CatalogueRow>): CatalogueRow {
  return {
    ...
    cluster: "pr-watch", gusWork: null, workUnitId: null, groupingId: null, statusLine: null, meta: {}, stage: null, activity: null, notes: null,
...
test("renders phase dot + clickable PR link + title", () => {
  const line = renderStatusline(
    row({ prNumber: 12080, prRepo: "heroku/dashboard", customTitle: "Fix navbar", stage: "in-review" }),
    { nowMs: NOW },
  );
...
test("strips a leading #num already baked into the title (no double PR#)", () => {
  const line = renderStatusline(
    row({ prNumber: 12080, prRepo: "heroku/dashboard", customTitle: "#12080 Fix navbar", stage: "building" }),
    { nowMs: NOW },
  );
...
test("adds the grouping label (linked) and W-number when both present", () => {
  ...
  const line = renderStatusline(
    row({ prNumber: 12080, prRepo: "heroku/dashboard", gusWork: "W-23392849", stage: "approved" }),
    { nowMs: NOW, grouping },
  );
...
test("a W-only session (no PR) uses the W-number as the label, not a trailing dup", () => {
  const line = renderStatusline(row({ gusWork: "W-23392849", stage: "building" }), { nowMs: NOW });
...
test("a stale row renders phase as unknown, never asserts the old value", () => {
  const stale = row({
    prNumber: 12080, prRepo: "heroku/dashboard", stage: "merged",
...
test("a fresh row within the window keeps its stage", () => {
  const line = renderStatusline(
    row({ prNumber: 1, prRepo: "a/b", stage: "merged", updatedAt: new Date(NOW - 1000).toISOString() }),
    { nowMs: NOW },
  );
...
test("no phase + fresh row: link only, no leading dot separator", () => {
  const line = renderStatusline(row({ prNumber: 5, prRepo: "a/b", stage: null }), { nowMs: NOW });
  expect(line.startsWith(" · ")).toBe(false);
  expect(line).toContain("#5");
});

test("a bare row with no PR/work reads as the neutral 'PR' label", () => {
  const line = renderStatusline(row({ stage: "building" }), { nowMs: NOW });
