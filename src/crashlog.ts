 ...
 * `<runtimeRoot>/crash.log`; breadcrumbs land in `ccs-debug.log` when CCS_DEBUG=1.
 *
 * Logs live at the runtime root (~/.ccs by default) rather than under cache/, so they survive
 * an `rm -rf ~/.ccs/cache` reset and stay easy to find.
 */
export const CRASH_LOG = () => join(runtimeRoot(), "crash.log");
export const DEBUG_LOG = () => join(runtimeRoot(), "ccs-debug.log");
