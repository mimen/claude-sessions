...
export function resumeClusterEntry(
  ...
  return resumeMany(indexDb, catalogueDb, sessionsForCluster(catalogueDb, cluster), { ...opts, cluster });
}
...
export function resumeMany(
  ...
  catalogueDb: Database,
  sessionIds: string[],
  opts: { dryRun?: boolean; cmuxBin?: string; bridge?: Bridge; focus?: boolean; cluster?: string } = {},
): ClusterResumeSummary {
  ...
  const shouldPin = (role: string | null): boolean => {
    ...
    let flag = false;
    try { flag = resolveRole(role, opts.cluster)?.pinOnResume === true; } catch { flag = false; }
